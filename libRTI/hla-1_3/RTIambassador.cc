...
/** Second Constructor for multiple RTIG */
RTI::RTIambassador::RTIambassador(std::string federate_name, int node, int _TotalNodes)
throw (RTI::MemoryExhausted, RTI::RTIinternalError)
{
  G.Out(pdGendoc,"enter RTIambassador::RTIambassador");
	PrettyDebug::setFederateName( "LibRTI::UnjoinedFederate" );
	
	/*   0-250    Nodes: 1 HLA CERVER is required (PORT: 60400)
	 *   250-500  Nodes: 2 HLA CERVERs are required (PORT: 60400 & PORT: 61400)
	 *   500-1000 Nodes: 3 HLA CERVERs are required (PORT: 60400 & PORT: 61400 & PORT: 62400)
	 */
	
	if (_TotalNodes > 250){
	  if ((federate_name.compare("GEM5") == 0) || (federate_name.compare("OMNET") == 0)){
	    if(node > 500){
	      setenv("CERTI_TCP_PORT","62400",1); // does overwrite
	      setenv("CERTI_UDP_PORT","62500",1); // does overwrite
	    }
	    else{
	      setenv("CERTI_TCP_PORT","61400",1); // does overwrite
	      setenv("CERTI_UDP_PORT","61500",1); // does overwrite
	    }
	  }
	  else{
	    setenv("CERTI_TCP_PORT","60400",1); // does overwrite
	    setenv("CERTI_UDP_PORT","60500",1); // does overwrite
	  }
	}
...
#else
	int pipeFd = privateRefs->socketUn->socketpair();
	printf("\n\n\n pipeFd=:%d | node:%d | federate_name:%s\n\n\n",pipeFd,node, federate_name.c_str());
	if (pipeFd == -1) {
		D.Out( pdError, "Cannot get socketpair to RTIA connection. Abort." );
		throw RTI::RTIinternalError( "Cannot get socketpair to RTIA connection" );
	}
